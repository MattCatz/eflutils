...
static char *core_storage = NULL;
static Elf *core = NULL;
static const char *exec = NULL;
static char *debuginfo_path = NULL;

static const Dwfl_Callbacks crash_callbacks = {
                 ...
                 void *arg __attribute__ ((unused)))
{
  ...
  const char *modname = dwfl_module_info (mod, NULL, NULL, &end, NULL, NULL,
                                          &mainfile, &debugfile);
  ...
  if (frames->frames == frames->allocated)
    {
      frames->allocated *= 2;
      frames->frame
          = realloc (frames->frame, sizeof (struct frame) * frames->allocated);
...
static void
print_frame (int nr, Dwarf_Addr pc, bool isactivation, Dwarf_Addr pc_adjusted,
             Dwfl_Module *mod, const char *symname, Dwarf_Die *cudie,
             Dwarf_Die *die)
{
  int width = get_addr_width (mod);
  ...
  const char *fname;
  Dwarf_Addr start;
  fname = dwfl_module_info (mod, NULL, &start, NULL, NULL, NULL, NULL, NULL);
  if (show_module)
    {
      if (fname != NULL)
        printf (" - %s", fname);
          ...
          if (dwarf_getsrcfiles (cudie, &files, NULL) == 0)
            {
              ...
              if (dwarf_formudata (dwarf_attr (die, DW_AT_call_file, &attr),
                  ...
                  == 0)
                {
                  ...
                  if (dwarf_formudata (
                          dwarf_attr (die, DW_AT_call_line, &attr), &val)
                      == 0)
                    {
                      line = val;
                      if (dwarf_formudata (
                              dwarf_attr (die, DW_AT_call_column, &attr), &val)
                          == 0)
                        col = val;
                    }
                }
  ...
  if (nscopes > 0)
    {
      /* scopes[0] == die, the lowest level, for which we already have
         ...
         happened.  */
      print_frame ((*nr)++, pc, isactivation, pc_adjusted, mod, symname, NULL,
                   NULL);
      ...
      for (int i = 1; i < nscopes && (maxframes == 0 || *nr < maxframes); i++)
        {
          ...
          if (tag != DW_TAG_inlined_subroutine && tag != DW_TAG_entry_point
...
print_frames (struct frames *frames, pid_t tid, int dwflerr, const char *what)
{
  ...
  int frame_nr = 0;
  for (int nr = 0;
       nr < frames->frames && (maxframes == 0 || frame_nr < maxframes); nr++)
    {
          ...
          if (show_debugname)
            {
              ...
              int nscopes
                  = dwarf_getscopes (cudie, pc_adjusted - bias, &scopes);
  ...
  if (frames->frames > 0 && frame_nr == maxframes)
    error (0, 0,
           "tid %lld: shown max number of frames "
           "(%d, use -n 0 for unlimited)",
           (long long)tid, maxframes);
  else if (dwflerr != 0)
    {
      if (frames->frames > 0)
        {
  ...
  switch (key)
    ...
    case ARGP_KEY_END:
      ...
      if (core == NULL) {
          ...
          if (core == NULL)
            error (EXIT_BAD, 0, "core(stdin) elf_memory: %s", elf_errmsg (-1));
      }
...
invoked with bad or missing arguments it will exit with return code 64.") };
  ...
  if (show_modules)
    {
      printf ("PID %lld - crash module memory map\n", (long long)dwfl_pid (dwfl));
